    ...
    class ClientTraceHelper
    {
        ...
        public void TraceRequestTimeout(EventId partitionEventId, uint partitionId)
        {
            if (this.logLevelLimit <= LogLevel.Warning)
            {
                ...
                if (EtwSource.Log.IsEnabled())
                {
                    EtwSource.Log.ClientRequestTimeout(this.account, this.taskHub, this.clientId, partitionEventId.ToString(), (int) partitionId, TraceUtils.ExtensionVersion);
                }
            }
        }
        ...
        public void TraceSend(PartitionEvent @event)
        {
            if (this.logLevelLimit <= LogLevel.Debug)
            {
                ...
                if (EtwSource.Log.IsEnabled())
                {
                    EtwSource.Log.ClientSentEvent(this.account, this.taskHub, this.clientId, @event.EventIdString, @event.ToString(), TraceUtils.ExtensionVersion);
                }
            }
        }
        ...
        public void TraceReceive(ClientEvent @event)
        {
            if (this.logLevelLimit <= LogLevel.Debug)
            {
                ...
                if (EtwSource.Log.IsEnabled())
                {
                    EtwSource.Log.ClientReceivedEvent(this.account, this.taskHub, this.clientId, @event.EventIdString, @event.ToString(), TraceUtils.ExtensionVersion);
                }
            }
        }
    }
