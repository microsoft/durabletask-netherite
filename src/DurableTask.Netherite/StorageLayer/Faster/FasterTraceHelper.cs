...
namespace DurableTask.Netherite.Faster
{
    ...
    class FasterTraceHelper
    {
        ...
        public void FasterAzureStorageAccessCompleted(string intent, long position, long size, string operation, string target, double latency, int attempt)
        {
            if (this.logLevelLimit <= LogLevel.Debug)
            {
                this.logger.LogDebug("Part{partition:D2} storage access completed intent={intent} position={position} size={size} operation={operation} target={target} latency={latency} attempt={attempt}",
                    this.partitionId, intent, position, size, operation, target, latency, attempt);
                EtwSource.Log.FasterAzureStorageAccessCompleted(this.account, this.taskHub, this.partitionId, intent, position, size, operation, target, latency, attempt, TraceUtils.AppName, TraceUtils.ExtensionVersion);
            }
        }
