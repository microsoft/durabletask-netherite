...
namespace DurableTask.Netherite.Faster
{
    ...
    class BlobEntry
    {
        ...
        ConcurrentQueue<Action<BlobUtilsV12.PageBlobClients>> pendingWrites;
        ...
        public BlobEntry(BlobUtilsV12.PageBlobClients pageBlob, AzureStorageDevice azureStorageDevice)
        {
            this.PageBlob = pageBlob;
            this.azureStorageDevice = azureStorageDevice;
            if (pageBlob.Default == null)
            {
                // Only need to allocate a queue when we potentially need to asynchronously create a blob
                this.pendingWrites = new ConcurrentQueue<Action<BlobUtilsV12.PageBlobClients>>();
                this.waitingCount = 0;
            }
        }
        ...
        /// for actual creation.
        /// </summary>
        public BlobEntry(AzureStorageDevice azureStorageDevice) : this(default, azureStorageDevice)
        {
        }
        ...
        public async Task CreateAsync(long size, BlobUtilsV12.PageBlobClients pageBlob)
        {
            ...
            await this.azureStorageDevice.BlobManager.PerformWithRetriesAsync(
                ...
                async (numAttempts) =>
                {
                    ...
                    await client.CreateAsync(
                        size: size,
                        cancellationToken: this.azureStorageDevice.PartitionErrorHandler.Token).ConfigureAwait(false);
            ...
            Action<BlobUtilsV12.PageBlobClients> action;
            // Clear actions
        ...
        public bool TryQueueAction(Action<BlobUtilsV12.PageBlobClients> writeAction)
        {
            int currentCount;
            do
            {
